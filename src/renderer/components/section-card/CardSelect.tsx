...
function ItemWrapper({ children }: PropsWithChildren) {
  return (
    <div className="flex items-center space-x-5 text-[0.8rem]">{children}</div>
  );
}

type CardSelectProps = {
  loading: boolean;
  selectedItem: ItemType | CP_FirstDepthCategory;
  itemList: ItemType[] | CP_FirstDepthCategory[];
  ...
  return (
    <div className="relative min-w-[100px]">
      <button
        ...
        type="button"
        className="cursor-default w-full rounded-md bg-neutral-400 dark:bg-neutral-700 pl-2 pr-8 shadow-box dark:shadow-box-dark ring-1 ring-amber-600 dark:ring-amber-500 focus:ring-2 hover:ring-2 text-sm"
      >
        <ItemWrapper>
